...
int comp_struct(const void *a,const void *b)
{
  Node *aa=(Node *)a;
  Node *bb=(Node *)b;
  return(((cabs(aa->value))<(cabs(bb->value)))?1:-1);
}
...
int comp_struct4(const void *a,const void *b)
{
  Pair *aa=(Pair *)a;
  Pair *bb=(Pair *)b;
  //if((aa->first)==(bb->first))
...
/*
  ...
  int num -- size of output, euqals to the cutoff num
  real_t *samples -- input array
  int n -- size of input array, in this case, is B
  Cut B into num
...
void find_largest_indices(int *output, int num, real_t *samples, int n){
  ...
  if (count < num){
    ...
    qsort(output, count, sizeof(int), compare);
  }
